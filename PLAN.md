# Company QA Task — Currency Conversion API Test Project

## Context

Take-home task for a QA Engineer position at Company: build an API test project against
`http://[task-pdf-host]` covering 3 mandatory E2E currency conversion
...
(no live web app). Re-check periodically; contact Company if it stays down. Final assertion
...
- **Business layer:** named asserter classes (e.g. `ConversionAsserter`) hold the
  ...
  still reads as "what is verified" (Company grades test-code clarity).
